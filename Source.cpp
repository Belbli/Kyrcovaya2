...
void enterRecordName(unsigned char key) {
	...
	printText(width / 2 + 100, height / 4, "¬ведите им€ рекорда : ");
	if (activeKey) {
		if (key == 8 && s > 0) {
			s--;
			record.name[s] = '\0';
...
void setLvlsInfo() {//—читывание кол-ва пройденных уровней и общего кол-ва уровней
...
void showRecords() {
	...
	if (recordsRows > 0){
		...
		printText(xPos, yPos, "–екордов не установлено");
